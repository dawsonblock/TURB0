...
| :--- | :--- | :--- | :--- |
| Llama runtime smoke | `fast-check` | `passed` | - |
| Gemma runtime smoke | `fast-check` | `passed` | - |
| Long-context stability | `fast-check` | `passed` | - |
| Dense vs paper_mse 2.75 bpc | `fast-check` | `passed` | - |
| Dense vs paper_mse 3.75 bpc | `fast-check` | `passed` | - |
| Dense-vs-TurboQuant sweep (llama) | `heavy-offline` | `passed` | - |
| paper_mse quality evaluation (llama) | `heavy-offline` | `passed` | - |
| Dense-vs-TurboQuant sweep (gemma) | `heavy-offline` | `passed` | - |
| paper_mse quality evaluation (gemma research-only) | `heavy-offline` | `captured` | Research-only observational tranche; this does not promote Gemma to a symmetric release guardrail.; Observed paper_mse gate outcomes: short:PASS, medium:PASS. |

KV rollup takeaway:
The retained bundle executed 5 fast-check stages and 3 heavy-offline stages.
...
Dataset selector: `scifact` with `512` docs and `64` queries.

| Method | Classification | Recall@1 | Recall@3 | Compression ratio | Query ms/query |
| :--- | :--- | ---: | ---: | ---: | ---: |
| `dense` | reference | 0.219 | 0.312 | 1.00x | 0.00 |
| `paper_mse` | paper-facing | 0.234 | 0.297 | 8.53x | 0.17 |
| `paper_prod_qjl` | paper-facing | 0.031 | 0.062 | 7.11x | 0.28 |
| `polarquant_exp` | supported non-paper-facing | 0.172 | 0.297 | 3.76x | 0.77 |
