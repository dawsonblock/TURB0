...
| Vector-search research lane | `benchmarks/vector_search/dataset_loader.py`, `benchmarks/vector_search/run_vector_search_eval.py`, `scripts/render_benchmark_snapshot.py` | `vector_search_summary.json`, `vector_search_metrics.csv`, `vector_search_summary.md`, dated history snapshots with optional vector-search sidecars | `partial` | `research-only` | The repo now has a bundled mini vector-search benchmark lane plus an explicit opt-in public-corpus path, but it remains research-only and not yet a broad retrieval claim. |
...
| `benchmarks/runtime_cert/run_dense_vs_tq.py`, `benchmarks/runtime_cert/run_kv_paper_eval.py` | KV-cache memory and throughput tradeoff on the allowlisted Apple-MLX path plus consolidated research reporting and Gemma observational `paper_mse` quality evidence | `product` lane and `research` lane | runtime-cert artifacts, `kv_paper_eval_summary.json`, `docs/history/BENCHMARK_SNAPSHOT_*.md` | The retained story is memory relief on a narrow hardware and family slice; throughput can regress materially, and Gemma `paper_mse` evidence remains observational rather than symmetric release gating. |
...
| `benchmarks/vector_search/run_vector_search_eval.py` | Vector-search recall, memory, and indexing claims on the bundled mini dataset and explicit public-corpus downloads | `research` lane | `vector_search_summary.json`, `vector_search_metrics.csv`, `vector_search_summary.md` | The bundled lane is intentionally small and the larger public-corpus lane is opt-in; neither one is a supported product surface. |
...
3. Whether the bundled mini vector-search lane and the explicit larger
   public-corpus lane generalize beyond these current research entry points
   without confusing retrieval research with the supported product lane.
