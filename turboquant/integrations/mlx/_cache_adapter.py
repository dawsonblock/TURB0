"""
turboquant.integrations.mlx._cache_adapter — internal MLX adapter layer.

This module provides the internal MLX adapter and helper utilities for
TurboQuant. MLX imports appear in multiple TurboQuant modules (kv_interface,
layout, pipeline, etc.) and are deferred to avoid breaking static analysis on
non-Apple platforms. This file is the canonical home for adapter-level MLX
helpers (version checking, dtype casting, eval/sync).
...
def dummy_quantize_main(x, *, config):
    ...
    *prefix, d = x.shape
    ...
    scales = mx.abs(x_groups).max(axis=-1, keepdims=True)
