"""Integration test: dense vs paper_mse (~3.75 average KV bpc) perplexity gate.

Uses the ``paper_mse`` preset
(``k_bits=3, k_group_size=64, v_bits=4, v_group_size=64``), which achieves
approximately 3.75 average KV bits per channel at ``d=128``.

With a real Llama model the quality gate is ``Δppl <= 0.5``, but that
...
def test_dense_vs_paper_mse_375bpc(tmp_path):
    """Dense vs paper_mse (~3.75 average KV bpc) perplexity comparison."""
    ...
    assert mean_delta <= max_delta_ppl, (
        ...
        f"at ~{_EXPECTED_AVG_BPC:.2f} average KV bpc (paper_mse preset)"
    )
