...
def test_single_entry_path_proof():
    ...
    and no legacy 'KVCompressor' or 'maybe_turboquant_k_cache' calls remain in production loops.
    """
                ...
                with open(fpath, "r") as f_handle:
                    ...
                    for pattern in legacy_patterns:
                        if pattern.search(content):
                            # Special case: generate.py is allowed to contain the legacy shim definition
                            if "generate.py" in rel_path and "def maybe_turboquant_k_cache" in content:
                                continue
                            pytest.fail(f"Legacy pattern '{pattern.pattern}' found in production file: {rel_path}")
